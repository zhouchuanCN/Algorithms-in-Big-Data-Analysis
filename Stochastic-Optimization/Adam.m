function [loss, w, state] = Adam(ObjFunc, x, y, w, state, opts)
%ADAM
...
% Correct for biased estimates of first and second moment
% vectors
state.m = state.m / (1 - beta1^state.t);
state.v = state.v / (1 - beta2^state.t);
...
w = w - lr * state.m ./ (state.v.^0.5 + epsilon);
